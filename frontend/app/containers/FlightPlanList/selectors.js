...
const makeSelectFlightPlans = () =>
  createSelector(selectFlightPlanListDomain, (state) => {
    ...
    const mapFn = (plan, i) =>
      plan
        ...
        .set(
          ...
          moment
            .unix(plan.get('planned_departure_time'))
            .utc()
        ...
        .set(
          ...
          moment
            .unix(plan.get('planned_arrival_time'))
            .utc()
            .format('YYYY-MM-DD HH:mm:ss')
        );
    ...
    if (flightPlansFilter === 'active') {
      return _.orderBy(
        state
          ...
          .filter((f) => f.get('state') === 'planned' || f.get('state') === 'completed')
          .map(mapFn)
          .toJS(),
        ['planned_departure_time'],
        ['desc']
...
const makeSelectFlightPlanList = () =>
  createSelector(selectFlightPlanListDomain, (substate) => substate.toJS());

export default makeSelectFlightPlanList;
export { selectFlightPlanListDomain, makeSelectFlightPlans };
